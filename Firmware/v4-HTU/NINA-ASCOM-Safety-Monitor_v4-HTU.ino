...
// IG: @eduardoziller
//
// v3-HTU — substituição do DHT11 pelo HTU21D + recuperação I²C automática:
//  - Sensor de temperatura/umidade: HTU21D via I²C (0x40)
//  - Eliminado barramento 1-wire do DHT11 (pino D0 liberado)
//  - Eliminada biblioteca DHTesp, adicionada Adafruit_HTU21DF
//  - Precisão melhorada: ±0.3°C e ±2% vs ±2°C e ±5% do DHT11
//  - RECUPERAÇÃO I²C: quando 2+ sensores falham simultaneamente,
//    o firmware executa a rotina de recuperação do barramento
//    (9 pulsos SCL + STOP + Wire.end/begin + reinit sensores)
//    Se falhar 15x sem recuperar, reinicia o ESP automaticamente
//  - Threshold de umidade reduzido para ±3% (HTU21D mais estável)
//  - Todas as correções da v2 mantidas: Watchdog HW, EEPROM magic byte,
//    validação NaN, I2C 50kHz, scan WiFi assíncrono, tendência suavizada
...
uint8_t falhasHTU = 0, falhasMLX = 0, falhasBMP = 0;
...
void setup() {
  ...
  // HTU21D — I²C, endereço fixo 0x40
  if (!htu.begin()) {
    Serial.println("AVISO: HTU21D nao encontrado. Verifique o I2C.");
  }
...
void logI2CEvento(uint8_t tentativa, bool reiniciando) {
  ...
  if (!f) return;
  if (f.size() == 0)
    f.println("DATA,HORA,TENTATIVA,HTU_FALHAS,MLX_FALHAS,BMP_FALHAS,SENSORES,RESULTADO");
  f.printf("%s,%s,%d,%d,%d,%d,%s,%s\n",
    dateBuf, timeBuf, tentativa,
    falhasHTU, falhasMLX, falhasBMP,
...
void recuperarI2C() {
  ...
  if (falhasHTU >= FALHAS_PARA_RECUPERAR) quem += "HTU";
  quem.trim();
  ...
  Serial.printf("║  MLX=%s  BMP=%s  HTU=%s\n",
    mlxOk ? "OK" : "FALHOU",
    bmpOk ? "OK" : "FALHOU",
    htuOk ? "OK" : "FALHOU");

  // Zera contadores para dar chance após a recuperação
  falhasMLX = 0; falhasBMP = 0; falhasHTU = 0;
...
// ─────────────────────────────────────────────────────────────────────────────
void lerSensores() {
  // HTU21D: leitura I²C, valida NaN, mantém último valor válido
  float novo_u = htu.readHumidity();
  float novo_t = htu.readTemperature();
  if (!isnan(novo_u) && !isnan(novo_t) && novo_u >= 0.0f && novo_u <= 100.0f) {
    u = novo_u;
    t = novo_t;
    falhasHTU = 0;
  } else {
    falhasHTU = min((int)falhasHTU + 1, 255);
  }

  // BMP280
  bmp.takeForcedMeasurement(); // Para o ESP comandar a leitura e evitar travamentos do barramento I2C 
  ...
  // Se 2+ sensores falhando simultaneamente = barramento provavelmente travado
  int sensoresFalhando = (falhasMLX >= FALHAS_PARA_RECUPERAR ? 1 : 0)
                       + (falhasBMP >= FALHAS_PARA_RECUPERAR ? 1 : 0)
                       + (falhasHTU >= FALHAS_PARA_RECUPERAR ? 1 : 0);
  if (sensoresFalhando >= 2) {
    recuperarI2C();
  } else if (sensoresFalhando == 0) {
    tentativasRecuperacao = 0; // tudo ok, zera o contador
  }
}
...
void tratarSerial() {
  ...
  if (agora - ultimaPublicacao >= 5000) {
    ...
    Serial.printf(
      ...
      falhasHTU, falhasMLX, falhasBMP   // contadores de falha para diagnóstico
    );
  }
}
...
void logData() {
  ...
  if (!exists) {
    f.println("DATA,HORA,TEMPERATURA,UMIDADE,TEMP_ORVALHO,PRESSAO,TIRCEU,"
              "ESTADO_NUVENS,ESTADO_CHUVA,CONDICAO_SEGURANCA,FALHAS_HTU,FALHAS_MLX,FALHAS_BMP");
  }
...
void iniciarServidor() {
  ...
  <body><div class="container">
    ...
    html += "<tr><td>Falhas HTU / MLX / BMP</td><td>" + String(falhasHTU) + " / " + String(falhasMLX) + " / " + String(falhasBMP) + "</td></tr>";
    html += "</table></div>";
  ...
  server.on("/espstatus", HTTP_GET, [](AsyncWebServerRequest *request) {
    ...
    page += "<tr><td>Falhas HTU21D</td><td>"   + String(falhasHTU) + "</td><td>" + badge(falhasHTU==0?"ok":falhasHTU<5?"warn":"crit") + "</td></tr>";
