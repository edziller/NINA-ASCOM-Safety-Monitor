...
// IG: @eduardoziller
//
// v4-DHT — versão DHT11 + BMP280 + MLX90614 atualizada com todas as melhorias:
//  - Recuperação I²C automática (9 pulsos SCL + STOP + Wire reinit)
//  - Log de eventos I²C em arquivo (/i2c_eventos.csv)
//  - Página web com card de saúde do barramento I²C
//  - Endpoint /i2clog para visualização em tempo real
//  - Clock I²C: 25kHz + setClockStretchLimit(25000) = 25ms
//  - Watchdog HW, EEPROM magic byte, validação NaN, tendência suavizada
//  - Threshold umidade: 5% (margem para ruído do DHT11)
...
// ── Recuperação I²C ───────────────────────────────────────────────────────────
#define FALHAS_PARA_RECUPERAR    3
#define MAX_TENTATIVAS_RESTART   3   // reinicia rapido — melhor que ficar em loop
...
void setup() {
  ...
  // I2C a 25kHz + clock stretch 25ms (adequado para MLX90614 + BMP280)
  Wire.begin();
  Wire.setClock(25000);
...
void logI2CEvento(uint8_t tentativa, bool reiniciando) {
  ...
  struct tm ti; localtime_r(&tnow, &ti);
  ...
  if (!f) return;
  if (f.size() == 0)
    f.println("DATA,HORA,TENTATIVA,MLX_FALHAS,BMP_FALHAS,SENSORES,RESULTADO");
...
void recuperarI2C() {
  ...
  if (falhasBMP >= FALHAS_PARA_RECUPERAR) quem += "BMP";
  quem.trim();
  ...
  Serial.printf( "║  Falhas: MLX=%d  BMP=%d\n", falhasMLX, falhasBMP);
  Serial.println("╠══════════════════════════════════════════════╣");
  ...
  Serial.println("║  Passo 3: Reinicializando Wire (25kHz)...");
  ...
  Serial.printf("║  MLX=%s  BMP=%s\n",
    mlxOk ? "OK" : "FALHOU", bmpOk ? "OK" : "FALHOU");

  falhasMLX = 0; falhasBMP = 0;
  ...
  Serial.println("║  Recuperacao concluida.");
  Serial.println("╚══════════════════════════════════════════════╝");
  Serial.println();
}
...
void lerSensores() {
  ...
  int sensoresFalhando = (falhasMLX >= FALHAS_PARA_RECUPERAR ? 1 : 0)
                       + (falhasBMP >= FALHAS_PARA_RECUPERAR ? 1 : 0);
  // DHT11 é 1-wire, não I²C — não conta para o gatilho I²C
...
void iniciarServidor() {
  ...
  <body><div class="container">
    ...
    // Card saude I2C
    String i2cBadge;
    if (totalRecuperacoes == 0)        i2cBadge = "<span style='color:#166534;font-weight:bold'>&#10003; Estavel</span>";
    else if (tentativasRecuperacao < 5) i2cBadge = "<span style='color:#9a3412;font-weight:bold'>&#9888; Instavel</span>";
    else                               i2cBadge = "<span style='color:#991b1b;font-weight:bold'>&#9888; CRITICO</span>";
    ...
    html += "<tr><td>Velocidade I2C</td><td>25 kHz</td></tr>";
  ...
  server.on("/i2clog", HTTP_GET, [](AsyncWebServerRequest *request) {
    String page = "<!DOCTYPE html><html><head><meta charset='UTF-8'><title>Log I2C</title>";
    page += "<meta name='viewport' content='width=device-width,initial-scale=1'>";
    page += "<meta http-equiv='refresh' content='15'>";
    page += "<style>body{font-family:Arial,sans-serif;background:#f4f7fb;margin:0;padding:20px}";
    page += ".container{max-width:1000px;margin:0 auto}.card{background:#fff;border-radius:14px;padding:18px;box-shadow:0 4px 14px rgba(0,0,0,.08);margin-bottom:20px}";
    page += "table{width:100%;border-collapse:collapse;font-size:13px}th{background:#1e3a8a;color:#fff;text-align:left;padding:8px}td{padding:7px 8px;border-bottom:1px solid #e5e7eb}";
    ...
    page += "<div class='container'><div class='card'><h1>Log de Eventos I&#178;C</h1>";
    page += "<a class='btn' href='/'>Voltar</a><a class='btn' href='/i2clog'>Atualizar</a>";
    page += "<p style='margin-top:12px;color:#6b7280;font-size:13px'>Total de recuperacoes desde o boot: <strong>" + String(totalRecuperacoes) + "</strong></p></div>";
    page += "<div class='card'><h2>Eventos registrados</h2>";
    File f = LittleFS.open("/i2c_eventos.csv", "r");
    if (!f || f.size() == 0) {
      page += "<p style='color:#166534;font-weight:bold'>&#10003; Nenhum evento — barramento estavel!</p>";
    } else {
      page += "<table><tr><th>Data</th><th>Hora</th><th>Tentativa</th><th>MLX falhas</th><th>BMP falhas</th><th>Sensores</th><th>Resultado</th></tr>";
      f.readStringUntil('\n');
      int count = 0;
      while (f.available()) {
        String linha = f.readStringUntil('\n'); linha.trim();
        if (linha.length() == 0) continue;
        String cols[7]; int pos = 0;
        for (int i = 0; i < 7; i++) {
          int next = linha.indexOf(',', pos);
          if (next < 0) next = linha.length();
          cols[i] = linha.substring(pos, next); pos = next + 1;
        }
        String rc = cols[6] == "RESTART_ESP" ? "crit" : "warn";
        page += "<tr><td>" + cols[0] + "</td><td>" + cols[1] + "</td><td>" + cols[2] + "</td>";
        page += "<td>" + cols[3] + "</td><td>" + cols[4] + "</td><td>" + cols[5] + "</td>";
        page += "<td class='" + rc + "'>" + cols[6] + "</td></tr>";
        count++;
      }
      page += "</table><p style='color:#6b7280;font-size:12px;margin-top:8px'>" + String(count) + " eventos.</p>";
    }
    if (f) f.close();
