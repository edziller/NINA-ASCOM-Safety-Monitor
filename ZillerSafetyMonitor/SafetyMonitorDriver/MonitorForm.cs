...
namespace ASCOM.EZZillerSafetyMonitor
{
    public partial class MonitorForm : Form
    {
        ...
        public MonitorForm(SafetyMonitorHardware hw)
        {
            ...
            txtAltitude.Text = hw.Altitude.ToString("F0");
            txtLimiteIR.Text = hw.LimiteIR.ToString("F1", CultureInfo.InvariantCulture);
            txtLimiteChuva.Text = hw.LimiteChuva.ToString();
        ...
        private void btnSalvar_Click(object sender, EventArgs e)
        {
            ...
            if (!float.TryParse(txtLimiteIR.Text.Trim(),
                                NumberStyles.Float, CultureInfo.InvariantCulture,
                                out float ir))
            {
                MessageBox.Show("Limite IR inválido");
                return;
            }
            if (!int.TryParse(txtLimiteChuva.Text.Trim(),
            ...
            if (!float.TryParse(txtLimiteDeltaP.Text.Trim(),
                                NumberStyles.Float, CultureInfo.InvariantCulture,
                                out float dp))
            {
            ...
            if (!float.TryParse(txtLimiteUmidade.Text.Trim(),
                                NumberStyles.Float, CultureInfo.InvariantCulture,
                                out float u))
            {
            ...
            if (!int.TryParse(txtAltitude.Text.Trim(),
                              NumberStyles.Integer, CultureInfo.InvariantCulture,
                              out int alt))
            {
        ...
        private void btnRedefinir_Click(object sender, EventArgs e)
        {
            ...
            txtLimiteDeltaP.Text = "-5.0";
            txtLimiteUmidade.Text = "85.0";
        }
